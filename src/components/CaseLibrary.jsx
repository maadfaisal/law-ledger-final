...
const CaseLibrary = () => {
  ...
  return (
    <section id="library" style={{ padding: '100px 10%', minHeight: '100vh', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
      <h2 className="font-head" style={{ fontSize: '2.5rem', marginBottom: '20px', color: '#e6f1ff' }}>
      ...
      <input 
        ...
        style={{
          ...
          color: 'white', marginBottom: '30px', fontFamily: 'Space Mono, monospace'
        }}
      />
      ...
      <div style={{ display: 'grid', gap: '20px' }}>
        {casesData.filter(c => c.title.toLowerCase().includes(search.toLowerCase())).map((item) => (
          <div key={item.id} onClick={() => toggle(item.id)} 
            style={{
              background: 'rgba(17, 34, 64, 0.7)', padding: '20px', borderRadius: '8px',
              border: '1px solid rgba(100, 255, 218, 0.1)', cursor: 'pointer', transition: '0.3s'
            }}
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ margin: 0, fontFamily: 'Playfair Display' }}>{item.title}</h3>
                <p style={{ color: '#8892b0', fontSize: '0.9rem', marginTop: '5px' }}>{item.citation}</p>
              </div>
              <span style={{ color: '#64ffda', fontSize: '1.5rem' }}>{openId === item.id ? "−" : "+"}</span>
            </div>

            {openId === item.id && (
              <div style={{ marginTop: '15px', paddingTop: '15px', borderTop: '1px solid #233554', color: '#a8b2d1' }}>
                {item.summary}
              </div>
            )}
          </div>
